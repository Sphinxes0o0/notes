# C/C++ 学习笔记

欢迎来到 C/C++ 学习笔记部分！这里包含了我在学习 C 和 C++ 语言过程中的各种笔记和总结。

## 目录结构

### C 语言
- [C 语言基础](./c/c.md) - C 语言的基本语法和概念
- [内存管理](./c/memory_management.md) - 内存分配和管理技术
- [内存相关](./c/memory.md) - 内存操作和优化

### C++
- [C++ 基础](./cpp/cpp.md) - C++ 语言的基本特性
- [嵌入式 C++](./cpp/embedded_cpp.md) - 嵌入式 C++ 开发
- [对象创建](./cpp/object_creation_heap_or_stack.md) - 堆栈对象创建策略

#### 容器
- [容器概览](./cpp/containers/containers_overview_guide.md) - STL 容器总览
- [vector](./cpp/containers/vector.md) - 动态数组
- [array](./cpp/containers/array.md) - 固定大小数组
- [list](./cpp/containers/list.md) - 双向链表
- [deque](./cpp/containers/deque.md) - 双端队列
- [stack](./cpp/containers/stack.md) - 栈
- [map](./cpp/containers/map.md) - 有序映射
- [unordered_map](./cpp/containers/unordered_map.md) - 无序映射

### 通用主题
- [位操作](./common_bit_operations.md) - 位运算技巧
- [编译过程](./compilation_process.md) - 程序编译流程
- [序列化](./serialization.md) - 数据序列化技术

---

这些笔记记录了我在学习过程中的理解和实践经验，希望能对大家有所帮助！

---
title: "26-门面模式如何实现-API-网关的高可用性"
description: "前面我们已经学习了组合模式、桥接模式、装饰模式、适配器模式这 4 种结构型设计模式，今天我们接着再来学习另一种新的结构型模式——门面模式。门面模式的原理非常容易理解，使用也非常灵活，因此，它的应用非常广泛。"
---
...
所以说，门面模式的原理本质就是简化外部系统使用内部多个子系统的使用方式。
使用场景分析

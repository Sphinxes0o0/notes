...
LwIP协议栈采用模块化设计，各模块通过精确定义的接口通信：

| 模块 | 功能 | 交互机制 |
|------|------|----------|
| 内存管理 | pbuf数据包缓冲区分配管理 | 静态内存池 + 动态内存分配 |
| 网络接口（netif） | 抽象不同类型的网络设备 | 驱动注册/回调机制 |
| ARP模块 | IPv4地址解析 | 定时器 + 缓存表 |
| 协议栈核心 | TCP/IP协议实现 | 分层架构，逐层封装 |
| 应用接口 | Socket API/Sequential API | Mailbox消息传递 |

> 各模块之间通过Mailbox机制进行通信，确保线程安全和高效的数据交换。
